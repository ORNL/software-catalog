...
function draw_pie_lines(areaID) {
  ...
  function drawGraph(data, areaID) {
    ...
    function clicked(d) {
      ...
      legend.transition(t).attr('fill-opacity', 0).attr('stroke-opacity', 0);

      titles.transition(t).attr('y', (d) => d.y + 30);
    ...
    function unclicked(d) {
      ...
      legend.transition(t).attr('fill-opacity', 1).attr('stroke-opacity', 1);

      titles.transition(t).attr('y', (d) => d.y);
    ...
    const textArray = [
      { text: graphHeader, class: 'graphtitle', x: 0, y: 0 },
      { text: d3.format('.4s')(dataTotalCount), class: 'graphtitle bignum', x: 0, y: -25 },
    ];
