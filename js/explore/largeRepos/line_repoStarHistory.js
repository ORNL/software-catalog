...
function draw_line_repoStarHistory(areaID) {
  ...
  function drawGraph(data, areaID) {
    ...
    var x = d3.scaleTime().domain(d3.extent(timerange)).range([0, width]);
    ...
    var yAxis = d3.axisLeft().scale(y).ticks(10, d3.format('d'));
    ...
    for (var repo in data) {
      chart.append('g').append('path').datum(data[repo]).attr('d', valueline).attr('stroke', color(repo)).attr('fill', 'none');
    }
  ...
  function reformatData(obj) {
    ...
    for (var repoObj of mostPopularRepositories) {
      ...
      for (var timestamp in starCounts) {
        data[`${repoObj.owner}/${repoObj.name}`].push({
          date: timestamp,
          value: starCounts[timestamp],
          name: `${repoObj.owner}/${repoObj.name}`,
        });
      }
    }
