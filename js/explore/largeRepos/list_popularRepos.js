...
function draw_popularRepos(areaID, columns = 2, orthogonalOrdering = false) {
  ...
  function drawList(data, areaID) {
    ...
    columns
      ...
      .html(
        (d) =>
          `<tspan style="font-weight: bold">${d.position}.</tspan> <a xlink:href=${window.location['origin']}/repo/#/${d.entry.owner}/${d.entry.name}>${d.entry.owner}/${d.entry.name}</a>`,
      );
