...
function draw_line_repoActivity(areaID, byCommits = true, repoNameWOwner) {
  ...
  Promise.all(files.map((url) => d3.json(url))).then((values) => {
    ...
    d3.select('.' + areaID)
      .append('g')
      .attr('transform', `translate(${980},${20})`)
      .call(slider);
  ...
  function drawGraph(data, areaID) {
    ...
    data = d3.stack().keys(repoKeys).order(d3.stackOrderAscending)(data);
    ...
    var areas = chart.append('g').attr('id', 'areas');
  ...
  function reformatData(obj0, obj1) {
    if (byCommits) {
      ...
      sortedTimestamps.forEach(function (timestamp) {
        var dateData = { date: timestamp };
        for (var repos of mostPopularRepositories) {
          dateData[`${repos['owner']}/${repos['name']}`] =
            repoData[timestamp][`${repos['owner']}/${repos['name']}`] == undefined
              ? 0
              : repoData[timestamp][`${repos['owner']}/${repos['name']}`];
        }
        data.push(dateData);
      });
    }
