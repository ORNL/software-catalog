...
function draw_line_repoActivity(areaID, repoNameWOwner) {
  ...
  function drawGraph(data, areaID) {
    ...
    function update(x, y) {
      ...
      // Draw fill
      chart.append('g').append('path').datum(data).attr('class', 'area').attr('d', area);

      // Draw line
      chart.append('g').append('path').datum(data).attr('class', 'line').attr('d', valueline);
      ...
      // Angle the axis text
      chart.select('.x.axis').selectAll('text').attr('transform', 'rotate(12)').attr('text-anchor', 'start');
    }
    ...
    function drawPie(weekData) {
      ...
      const pieGroup = chart.append('g').attr('transform', `translate(${(5 * width) / 6},${height / 2})`);
      ...
      label.nodes().forEach((node) => {
        node.setAttribute(
          ...
          Math.min(
            11,
            Math.floor((11 * radius) / node.getComputedTextLength()),
            Math.floor((Math.PI * radius * node.getAttribute('h')) / 2),
          ) + 'px',
        );
      });
  ...
  function reformatData(obj, infoObj) {
    ...
    var repoCounts = {};
    var repos =
      repoNameWOwner == null ? Object.keys(obj['data']).filter((d) => Object.keys(infoObj['data']).some((o) => o == d)) : [repoNameWOwner];
