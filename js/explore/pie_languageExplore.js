...
function draw_pie_language(areaID) {
  ...
  function drawGraph(data, areaID) {
    ...
    const textArray = [
      { text: graphHeader, size: 18, weight: 'bold', id: 'mainTitle', x: 0, y: 0 },
      { text: 'by bytes', size: 18, weight: 'normal', id: 'subTitle', x: 0, y: '1em' },
    ];
  ...
  function reformatData(obj) {
    ...
    Object.keys(obj['data']).forEach((repoName) => {
      ...
      for (var i = 0; i < value; i++) {
        repoData[repoName].push({
          name: obj['data'][repoName]['languages']['nodes'][i]['name'],
          size: obj['data'][repoName]['languages']['edges'][i]['size'],
          reference: { number: 1, key: obj['data'][repoName]['languages']['nodes'][i]['name'] },
        });
      }
